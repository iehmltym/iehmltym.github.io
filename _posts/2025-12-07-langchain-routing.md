...
面向读者：**中级**。场景：**生产级客服 Agent 成本控制**。

## 背景/问题
所有问题都用大模型成本太高。需要按问题复杂度路由到不同模型。
...
judge_prompt = PromptTemplate.from_template(
    "判断问题复杂度，仅输出 easy 或 hard：{question}"
)
llm = ChatOpenAI(model="gpt-4o-mini", temperature=0)

print((judge_prompt | llm).invoke({"question": "退款多久到账？"}).content)
```

**意图与边界**：分类路由；边界是模型判断可能不稳定。
...
def route(label: str):
    return ChatOpenAI(model="gpt-4o-mini" if "easy" in label else "gpt-4o", temperature=0.2)

label = "easy"
llm = route(label)
print(llm.invoke("用一句话回答：如何重置密码？").content)
```

**意图与边界**：实际路由；边界是标签错误会走错模型。

## 常见坑与排错
- 分类 prompt 太模糊，导致大量误判。
- 缺少默认回退模型。
