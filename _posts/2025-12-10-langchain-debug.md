...
面向读者：**中级**。场景：**生产级客服 Agent 输出不稳定**。

## 背景/问题
输出异常往往不是“模型坏了”，而是某个步骤输入异常。需要可观测性。
...
**示例 2：打印完整 prompt（可运行）**

```python
from langchain_core.prompts import PromptTemplate

prompt = PromptTemplate.from_template("问题：{q}\n请给出答案。")
print(prompt.format(q="如何解绑银行卡？"))
```

**意图与边界**：定位提示词问题；边界是未结合模型输出。
